...
pair<int,int> parseSeatID(const string& seatID) {
    ...
    int col = find(COL_LETTERS.begin(), COL_LETTERS.end(), colChar) - COL_LETTERS.begin();
    int row = stoi(seatID.substr(1)) - 1;
    if (col < 0 || col >= COLS || row < 0 || row >= ROWS) return {-1,-1};
    return {row, col};
}

// get class name from row index\Nstring seatClass(int row) {
...
// display seating chart\Nvoid displaySeatingChart() {
    cout << "\n    A   B   D   E   F   H\n";
...
// save current state to file\Nvoid saveDataToFile() {
    ofstream out("seats.txt");
    for (int r=0; r<ROWS; ++r) for (int c=0; c<COLS; ++c) {
        if (seats[r][c]==TAKEN)
            out<<COL_LETTERS[c]<<(r+1)<<":"<<names[r][c]<<"\n";
    }
}
// load state from file\Nvoid loadDataFromFile() {
...
void automaticChooseSeat(const string& user) {
    ...
    for (int tries=0; tries<ROWS*COLS; ++tries) {
        ...
        if (seats[r][c]==EMPTY) {
            seats[r][c]=TAKEN;
            names[r][c]=user;
            cout<<"Assigned seat "<<COL_LETTERS[c]<<(r+1)<<" ("<<seatClass(r)<<")\n";
            assigned=true;
            saveDataToFile();
            break;
        }
...
void printManifest() {
    ...
    for(int r=0;r<ROWS;++r) for(int c=0;c<COLS;++c)
        if (seats[r][c]==TAKEN)
            list.push_back({names[r][c], string()+COL_LETTERS[c]+to_string(r+1)});
    cout<<"Sort by Seat (S), Name (N), or Class (C): "; char opt = toupper(readLine()[0]);
    if (opt=='N') {
    ...
    // class grouping: just natural order by row
    cout<<"MANIFEST:\n";
    for (auto &p: list)
...
void reassignOrCancelAnySeat() {
    cout<<"Type C to cancel or R to reassign: "; char op = toupper(readLine()[0]);
    cout<<"Enter seat ID: "; string sid=readLine(); auto [r,c]=parseSeatID(sid);
    if (r<0 || seats[r][c]==EMPTY) { cout<<"No reservation there.\n"; return; }
    string user = names[r][c]; seats[r][c]=EMPTY; names[r][c].clear();
...
void passengerMenu(const string& user) {
    while (true) {
        cout<<"\n1) Show chart  2) Auto-choose  3) Manual  4) Change/Cancel  5) Boarding Pass  6) Quit\n";
...
void employeeMenu() {
    ...
    if (u!="admin"||p!="password") { cout<<"Auth failed.\n"; return; }
    loadDataFromFile();
    while (true) {
        cout<<"\nE) Show chart  M) Manifest  L) Load  S) Save  R) Reassign/Cancel  Q) Quit\n";
